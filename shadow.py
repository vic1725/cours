...
cmdUtils.register_command("shadow_property", "<str>",
                          ...
                          default="color")
cmdUtils.register_command("is_ci", "<str>",
                          "If present the sample will run in CI mode (optional, default='None'. Will publish shadow automatically if set)")
    ...
    try:
        ...
        try:
            if response.state.reported != None:
                ...
                print(
                    "Shadow states cleared.")  # when the shadow states are cleared, reported and desired are set to None
...
def change_shadow_value(value):
    with locked_data.lock:
        ...
        if value == "clear_shadow":
            tmp_state = iotshadow.ShadowState(reported=None, desired=None, reported_is_nullable=True,
